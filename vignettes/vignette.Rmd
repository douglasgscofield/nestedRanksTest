...
The nested ranks test was first described in detail in the supplemental
...
* a formula interface, the easiest to use, takes a formula of the form `y ~ x | groups` and includes `data` and `subset` arguments
...
the formula `y ~ x`.  All of the following are valid and produce identical results.
...
```{r, eval = FALSE}
## Error, no grouping specified
result <- nestedRanksTest(Distance ~ Year, Distance, data = d.Qlob)
## Error, grouping specified twice
...
#
#
nestedRanksTest(Distance ~ Year | Granary, data = d.Qagr, n.iter = 2000)
```
...
```{r, fig.width = 6, fig.height = 3}
plot(result.Ql, main = expression(italic("Quercus lobata")), col = "lightgreen", 
...
Scofield, D. G., Sork, V. L., and Smouse, P. E.  (2010)  Influence of acorn woodpecker social behaviour on transport of coast live oak (*Quercus agrifolia*) acorns in a southern California oak savanna.  [*Journal of Ecology* **98**: 561-572][Scofield2010].

Thompson, P. G., Smouse, P. E., Scofield, D. G., and Sork, V. L. (2014)  What seeds tell us about birds: a multi-year analysis of acorn woodpecker foraging movements.  [*Movement Ecology* **2**: 12][Thompson2014].
