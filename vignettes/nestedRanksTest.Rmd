...
tables to see how big our sample sizes are.
...
are too small for convincing single-granary results, and it does not fit the
scale of our original question:  are there *overall* differences in seed
movement between high- and low-crop years?  We could find or develop a
technique to combine the p-values, but this still feels off the mark.
...
Mann-Whitney-Wilcoxon test statistic is then computed.  Because the scale of
this statistic is sample-size dependent (between -*n*<sub>1</sub> &times;
*n*<sub>2</sub> and +*n*<sub>1</sub> &times; *n*<sub>2</sub>), it is scaled to
[-1, +1] by dividing by *n*<sub>1</sub> &times; *n*<sub>2</sub>.
...
dividing each by their sum.  Thus the weights sum to 1.
...
sample sizes in each treatment and weights.
